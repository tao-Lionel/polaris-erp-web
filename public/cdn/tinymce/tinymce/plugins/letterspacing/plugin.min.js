...
tinymce.PluginManager.add("letterspacing", function(a, l) {
  function g(b) {
    const e = a.dom;
    const f = a.selection.getSelectedBlocks();
    h.each(f, function(c) {
      if (e.getStyle(c, "text-indent")) {
        let d = "";
        c &&
          c.children["0"] &&
          c.children["0"].attributes &&
          c.children["0"].attributes.style &&
          (d = (d = (d = c.children["0"].attributes.style.textContent.match(/font-size:?(.+?)"?[;}]/)) ? d[1] : !1)
            ? `${2 * (parseInt(d) + parseInt(b || 0))}px`
            : `${2 * (parseInt(b || 0) + 16)}px`);
        e.setStyle(c, "text-indent", d || "2em");
      }
    });
  ...
  const k = a.getParam("letterspacing", "0px 1px 2px 4px 6px 8px 10px 20px 40px");
  a.on("init", function() {
    a.formatter.register({ letterspacing: { inline: "span", toggle: !1, styles: { "letter-spacing": "%value" }, clear_child_styles: !0 } });
  });
  a.ui.registry.getAll().icons.letterspacing ||
  ...
  a.ui.registry.addMenuButton("letterspacing", {
    icon: "letterspacing",
    tooltip: "\u8bbe\u7f6e\u95f4\u8ddd",
    fetch(b) {
      const e = k.split(" ").map(function(f) {
        return {
          type: "togglemenuitem",
          text: f,
          onAction() {
  ...
  return {
    getMetadata() {
      return { name: "\u8bbe\u7f6e\u95f4\u8ddd", url: "https://github.com/Five-great/tinymce-plugins" };
    }
  };
});
