<template>
  <el-dialog title="取消shipment" :visible.sync="showDialog" width="80%" :before-close="handleClose">
    <div>
      <p style="color: red; font-size: 15px; margin-bottom: 10px">
        <span v-if="loading">正在处理数据，请不要进行任何操作，不要关闭此页面，谢谢！</span>
        <span v-else>取消shipment已处理完毕！</span>
      </p>
...
<script>
import { cancelShipmentApi } from "@/api/order/batchInvoiceManage"
...
export default {
  ...
  methods: {
    async openDialog(list, type, batchSize = 5) {
      try {
        ...
        for (const batch of batches) {
          const promises = batch.map(async (item) => {
            try {
              const { data } = await cancelShipmentApi({
                foBatchOrderId: item.id,
                forceCancelFlag: String(type)
              })
              return {
                orderCode: item.batchOrderCode,
                code: data.code,
