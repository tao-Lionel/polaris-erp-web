  ...
  <div>
    ...
    >
      ...
      <template slot="menuLeft">
        ...
        <el-button v-if="permissions.finance_bill_overseas_delete" type="primary" size="small" @click="handleDelete()">
          删除
        </el-button>
        <export-btn
          v-if="permissions.finance_bill_overseas_export"
          :pageName="pageName"
          pageUrl="/finOverseaWarehouseBill/pageList"
          moduleName="finance"
...
import { getOverseasBillPageListApi, deleteOverseaBillApi } from "@/api/finance/billingManagement"
...
export default {
  name: "BillingManagementOverseasBillTable",
  ...
  methods: {
    async init() {
      await Promise.all([this.getAllCountry(), this.getList(), this.getAllSupplier()])
    },
    ...
    async getList(column = null) {
      ...
      const { data } = await getOverseasBillPageListApi(params)
      ...
      return [
        ...
        },
        {
          ...
          clickFn: (scope) => {
            ...
            console.log(getTokenUrl(scope.row.excelFileUrl))
        ...
        },
        {
          ...
          show: this.permissions.finance_bill_overseas_delete
        }
      ]
    },
    handleDetail(row) {
      this.$emit("switchTab", "", { settlementNo: row.settlementNo })
    },
    getSummaries(param) {
    ...
    handleDelete(ids = null) {
      ...
      this.$confirm("删除后将无法恢复，确定要删除吗？", "提示", {
        ...
        const { data } = await deleteOverseaBillApi({ billCodes: ids })
        if (data.code === 0) {
