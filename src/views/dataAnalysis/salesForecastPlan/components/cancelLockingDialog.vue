<template>
  <el-dialog title="取消锁定销量预测" :visible.sync="showCancelLockingDialog" width="500px" center>
    <div v-show="!searchFormConfirm">
      您勾选了
      <span style="font-size: 18px; color: red">{{ this.selectList.length }} </span>条数据
    </div>

    <div>确定将销量预测的状态回退至"未锁定"吗？</div>
    <el-form ref="form" :model="form" :rules="rules">
      <el-form-item label="取消锁定的月份" prop="date">
...
import { getUnLock } from "@/api/dataAnalysis/salesForecastPlan"

export default {
  name: "cancelLockingDialog",
  components: {},
  props: {},
  data() {
    return {
  ...
  methods: {
    openDialog(selectList, searchFormConfirm = null, list) {
      this.showCancelLockingDialog = true
    ...
    submitForm() {
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          ...
          await getUnLock({ ids, unlockDates: this.form.date, queryPageDto })
    ...
    cancel() {
      console.log("取消")
      this.showCancelLockingDialog = false
    }
  },
  watch: {}
}
