    ...
    <basic-container>
      ...
      >
        ...
        <template slot="menuLeft">
          <el-button type="primary" v-if="permissions.sales_forecast_plan_import" @click="handleImport" size="small">
            导 入
          </el-button>

          <el-dropdown v-if="permissions.sales_forecast_plan_lock" style="margin-right: 10px">
          ...
          <el-dropdown v-if="permissions.sales_forecast_plan_cancel_lock" style="margin-right: 10px">
            <el-button type="primary" size="small">
          ...
          <el-button type="primary" v-if="permissions.sales_forecast_plan_export" @click="handleExport" size="small">
            导出
          </el-button>

          <el-dropdown v-if="permissions.sales_forecast_plan_delete" style="margin-right: 10px">
            <el-button type="primary" size="small">
        ...
        <template slot="linkSku" slot-scope="{ row }">
          <copy-text hoverToShow>
            <a v-if="permissions.sales_forecast_plan_check" @click="handleCheck(row)">{{ row.linkSku }}</a>
...
} from "@/api/dataAnalysis/salesForecastPlan"
...
export default {
  ...
  methods: {
    ...
    async customDownload() {
      ...
      downloadBlob(data, "销量预测导入模板(备货版).xlsx", "application/vnd.ms-excel;charset=utf-8")
    },
    // 导入数据
    importExcelBtn(file) {
      if (!file) {
