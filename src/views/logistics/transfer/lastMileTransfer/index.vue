      ...
      <basic-container>
        ...
        >
          <template slot="menuLeft">
            ...
            <el-button
              type="primary"
              v-if="permissions.warehouse_transfer_final_detail_export"
              @click="exportData"
              size="small"
              >导出</el-button
            >
          ...
          <template slot="searchAdvanced">
            <el-form @submit.prevent.native :model="searchForm" :inline="true" ref="searchForm">
              ...
              <el-form-item prop="thirdPartyInNo">
                <avue-crud-input
                  v-model="searchForm.thirdPartyInNo"
                  size="small"
                  placeholder="海外仓/平台仓入库单号,支持多个"
                  v-search-input
                ></avue-crud-input>
              </el-form-item>
              <el-form-item prop="thirdPartyOutNo">
                <avue-crud-input
                  v-model="searchForm.thirdPartyOutNo"
                  size="small"
                  placeholder="海外仓/平台仓出库单号,支持多个"
                  v-search-input
                ></avue-crud-input>
              </el-form-item>
            ...
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item
                  ...
                  size="small"
                  plain
                  @click.native="costAdjustRecord(row)"
                  v-if="
                    row.costLockStatus === 'LOCK' &&
                    row.modifycostLockStatus === 'UN_LOCK' &&
                    row.transferStatus !== 'CANCEL'
                  "
                  >录入调整费用</el-dropdown-item
                >
                <el-dropdown-item
                  type="text"
                  size="small"
          ...
          <template slot="transferNo" slot-scope="scope">
            <copy-text hoverToShow>
              <router-link :to="'/transfer/lastMileTransfer/detail?id=' + scope.row.id + '&type=detail'">{{
          ...
          <template slot="relationNo" slot-scope="scope">
            ...
            <br />
            <span>海外仓/平台仓出库单号：{{ scope.row.thirdPartyOutNo }}</span
            >&nbsp;<el-button v-if="scope.row.thirdPartyOutNoHand" type="primary" size="mini">人工关联单号</el-button>
            <br />
            <span>海外仓/平台仓预约入库单号：{{ scope.row.thirdPartyAsnInNo }}</span
            >&nbsp;<el-button v-if="scope.row.thirdPartyAsnInNoHand" type="primary" size="mini">人工关联单号</el-button>
            <br />
            <span>海外仓/平台仓入库单号：{{ scope.row.thirdPartyInNo }}</span
          ...
          <template slot="timePoint" slot-scope="scope">
            <span>预计出库时间：{{ scope.row.estimateOutStockTime }}</span>
            <br />
            <span>预计到达时间：{{ scope.row.estimateArrivalTime }}</span>
            <br />
        ...
        <el-dialog title="人工关联单号" :visible.sync="relatedDialog" width="500px" center>
          <div style="padding: 0 10px">
            ...
            <el-form :model="relatedForm" style="width: 60%px">
              <el-form-item label="海外仓/平台仓出库单号:" style="margin-bottom: 5px">
                <el-input v-model="relatedForm.thirdPartyOutNo" size="small"></el-input>
              </el-form-item>
              <el-form-item label="海外仓/平台仓预约入库单号:">
                <el-input v-model="relatedForm.thirdPartyAsnInNo" size="small"></el-input>
              </el-form-item>
              <el-form-item label="海外仓/平台仓入库单号:">
              ...
              <template #default="props">
                <el-table :data="props.row.confirmDetails" style="width: 100%">
                  ...
                  <el-table-column label="本次入库数量">
                    <template slot-scope="{ row }">
                      <el-input v-model="row.inCount" type="number" size="mini"></el-input>
                    </template>
                  </el-table-column>
                </el-table>
              </template>
        ...
        <el-dialog title="录入费用" :visible.sync="recordDialog" width="80%" center>
          <el-form :model="recordData.enterCost" ref="recordForm" :rules="costRules">
            <el-table :data="costObj.costInfo">
              <el-table-column label="物流商" prop="transportCode">
                <template slot-scope="{ row }">
                  <el-select
                    size="mini"
                    clearable
              ...
              <el-table-column label="运费" prop="cost">
                <template slot-scope="{ row }">
                  <div style="display: flex; align-items: center">
                    <el-input
                      ...
                      :disabled="!row.isOperate"
                    ></el-input>
                    <el-select v-model="row.costCurrency" size="mini" :disabled="!row.isOperate">
                      <el-option
                ...
                <template slot-scope="scope">
                  <div>
                    <el-button
                      ...
                      @click="addCost(scope.row)"
                      v-if="scope.$index === costObj.costInfo.length - 1"
                      >添加</el-button
                    >
                    <el-button
                      type="text"
                      style="padding: 10px 0; color: rgb(140, 9, 9)"
                      @click="deleteCost(scope.row, scope.$index)"
                      >删除</el-button
                    >
                  </div>
                </template>
              ...
              <!-- <el-table-column label="清关费" prop="clearanceCost">
                <template slot-scope="{row}">
                  <div style="display: flex; align-items: center;">
                    <el-input v-model="row.transferTransportCostDetailVOS[0].cost" size="mini" type="number"
                      :disabled="row.transferTransportCostDetailVOS[0].costLockStatus == 'LOCK'"></el-input>
                    <el-select v-model="row.transferTransportCostDetailVOS[0].costCurrency" size="mini"
                      :disabled="row.transferTransportCostDetailVOS[0].costLockStatus == 'LOCK'">
              ...
              <el-table-column label="关税费">
                <template slot-scope="{row}">
                  <div style="display: flex; align-items: center;">
                    <el-input v-model="row.transferTransportCostDetailVOS[3].cost" size="mini" type="number"
                      :disabled="row.transferTransportCostDetailVOS[3].costLockStatus == 'LOCK'"></el-input>
                    <el-select v-model="row.transferTransportCostDetailVOS[3].costCurrency" size="mini"
                      :disabled="row.transferTransportCostDetailVOS[3].costLockStatus == 'LOCK'">
                      <el-option v-for="item in currencyOptions" :key="item.value" :label="item.label"
                        :value="item.value"></el-option>
                    </el-select>
                  </div>
              ...
              <el-table-column label="其他费用" prop="otherCost">
                <template slot-scope="{row}">
                  <div style="display: flex; align-items: center;">
                    <el-input v-model="row.transferTransportCostDetailVOS[1].cost" size="mini" type="number"
                      :disabled="row.transferTransportCostDetailVOS[1].costLockStatus == 'LOCK'"></el-input>
                    <el-select v-model="row.transferTransportCostDetailVOS[1].costCurrency" size="mini"
                      :disabled="row.transferTransportCostDetailVOS[1].costLockStatus == 'LOCK'">
                      <el-option v-for="item in currencyOptions" :key="item.value" :label="item.label"
                        :value="item.value"></el-option>
                    </el-select>
                  </div>
          ...
          <span slot="footer" class="dialog-footer">
            <el-button size="small" type="primary" @click="saveRecord" :disabled="buttonLoading">保存</el-button>
          ...
          <el-table :data="recordData.enterCost">
            ...
            <el-table-column label="运费" prop="freightCost">
              <template slot-scope="{ row }">
                <div style="display: flex; align-items: center">
                  <el-input
                    v-model="row.transferTransportCostDetailVOS[3].cost"
                    size="mini"
                    type="number"
                    :disabled="row.transferTransportCostDetailVOS[3].costLockStatus == 'LOCK'"
                  ></el-input>
                  <el-select
                    v-model="row.transferTransportCostDetailVOS[3].costCurrency"
                    size="mini"
                    :disabled="row.transferTransportCostDetailVOS[3].costLockStatus == 'LOCK'"
                  >
                    <el-option
...
import {
  ...
  handleCost,
  transferFinalExportExcelApi
} from "@/api/logistics/transfer/fristMileTransfer"

import { lastTableOption } from "@/const/crud/logistics/transfer/fristMileTransfer"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      //搜索条件，和前端控件绑定
      searchForm: {
        transferType: ["FOOT_O2P", "FOOT_O2O", "FOOT_P2O"],
        transferNo: "", //调拨单号
        fmWarehouseId: [], //转出仓
        toWarehouseId: [], //目的仓
        fmCountryCode: [], //起运国家
  ...
  methods: {
    init() {
      ...
      getObjByType("in_abnormal_type").then((res) => {
        let data = res.data.data.filter((item) => item.id == 407 || item.id == 408)
        this.inAbnormalOptions = data
      })
      //调拨状态
      ...
      getObjByType("transfer_type").then((res) => {
        if (res.data.code === 0) {
          this.transferTypeArr = res.data.data.filter((item) => item.value !== "HEAD")
        }
      })
      //费用状态
    ...
    editTransfer(row) {
      console.log("row", row)
      this.$router.push({
        path: "/transfer/lastMileTransfer/edit",
    ...
    confirmInStorage() {
      ...
      if (isValid) {
        ...
        confirmPutInStorage({
          ...
          transferDetails: this.instockData
        }).then((res) => {
    ...
    async costRecord(row) {
      // getCostInfoById({ transferId: row.id }).then((res) => {
      ...
      //   });
      //   this.recordData = {
      //     enterCost: res.data.data.transferCostDetailVOS,
      //     transferId: res.data.data.transferId,
      //     saveModel: res.data.data.transferCostDetailVOS[0].costOperateType,
      //   };
      //   console.log(this.recordData.enterCost);
      // });
    ...
    async saveRecord() {
      // this.recordData.enterCost.forEach((item) => {
      //   item.transferTransportCostDetailVOS = [item.shipment];
      // });

      // let obj = {
      //   enterCost: this.recordData.enterCost.flatMap(
      //     ({ transferTransportCostDetailVOS, ...rest }) =>
    ...
    costAdjustRecord(row) {
      getLockCostInfoById([row.id]).then((res) => {
        ...
        } // this.recordData = res.data.data.costDetails
        // if (this.recordData.length <= 0 || this.recordData.every(item => item.costLockStatus == 'LOCK')) {
        //   this.recordData.push({
        //     transferId: res.data.data.transferId,
        //     forwarderTransportName: res.data.data.forwarderTransportName,
        //     clearanceTransportName: res.data.data.clearanceTransportName,
        //     sendTransportName: res.data.data.sendTransportName,
        //     freightCost: 0,
        //     freightCurrency: "",
        //     clearanceCost: 0,
        //     clearanceCurrency: "",
    ...
    async saveAdjustRecord() {
      // let isValid = this.validateAdjustData()
      let isValid = true
      console.log(isValid)
    ...
    relatedNo(row) {
      ...
      this.relatedForm.thirdPartyOutNo = row.thirdPartyOutNo
    ...
    getList(page, params) {
      ...
      console.log(this.searchForm, 10223)
      if (!this.searchForm.transferType || this.searchForm.transferType.length == 0) {
        this.searchForm.transferType = ["FOOT_O2P", "FOOT_O2O", "FOOT_P2O"]
      }
      this.handleSearchFilter()
      console.log(this.searchFormConfirm, 930)
      fetchList(
        Object.assign(
          {
    ...
    handleSearchFilter() {
      //将搜索的条件放入到确认的搜索条件中
      console.log(101, this.searchForm)
      this.searchFormConfirm = deepCloneIgnoreNull(this.searchForm)
      console.log(102, this.searchFormConfirm)
      this.searchFormConfirm.filtering = this.transformSearchForm(this.searchFormConfirm)
      console.log(103, this.searchFormConfirm)
    ...
    openNewPage: function () {
      this.$router.push({ path: "/transfer/lastMileTransfer/add?type=add" })
    },
    //查看操作日志
    ...
    getCellClass({ row }) {
      if (row.abnormalType != "NOT_ABNORMAL") {
        return "warningRow"
      }
    },
    addCost(row) {
    ...
    deleteCost(row, index) {
      ...
      //   return
      // }
      this.costObj.costInfo.splice(index, 1)
    },
