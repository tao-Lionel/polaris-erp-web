        ...
        <el-form label-width="100px" :model="detailForm" ref="detailForm">
          <detail-wrapper :options="{ title: '基本信息' }">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="暂估应付单：" label-width="110px">
                  <div style="display: flex">
                    <span style="margin-left: 10px">{{ detailForm.fEstimatedBillNo }}</span>
                    <router-link
                      :to="{
                        path: '/financialStatements/cost/costAdjustmentDetail/psdetail/',
                        query: { fBillNo: detailForm.fEstimatedBillNo }
                      }"
                    >
                      {{ item }}
                    </router-link>
        ...
        <detail-wrapper :options="{ title: '费用明细' }">
          <div style="padding: 10px">
            <avue-crud
              ...
              :option="accountsPayableDetailOption"
              :summaryMethod="getSummaries"
            >
              <template slot-scope="scope" slot="costAdjustBillNoList">
                <p v-for="item in scope.row.costAdjustBillNoList" :key="item">
                  <router-link
...
import { accountsPayableDetailOption, logOption } from "@/const/crud/financialStatements/index"
...
export default {
  ...
  data() {
    return {
      ...
      accountsPayableDetailOption: accountsPayableDetailOption,
      logOption: logOption,
      logData: [],
      activeName: "first"
    }
  ...
  methods: {
    getSummaries(param) {
      ...
      const sums = []
      const amountKey = ["fTaxPrice", "requiredAdjustedCost"]
    ...
    getDetailInfo() {
      ...
      getDetail({ fBillNo: fBillNo }).then((res) => {
        if (res.data.code === 0) {
          this.detailForm = res.data.data.financialPayAbleDetailVo
        }
      })
    }
  },
