    ...
    <basic-container>
      ...
      >
        ...
        <template slot="menuLeft">
          <el-button v-if="permissions.safety_stock_add" type="primary" @click="handleAdd" size="small">
            新 增
          </el-button>
          <el-button v-if="permissions.safety_stock_import" type="primary" @click="handleImport" size="small">
            导 入
          </el-button>
          <el-button v-if="permissions.safety_stock_export" type="primary" @click="handleExport" size="small">
            导 出
          </el-button>
        </template>
        <!-- 搜索栏 -->
        ...
        <template slot="operate" slot-scope="scope">
          <a v-if="permissions.safety_stock_edit" @click="handleEdit(scope.row)">编辑</a>
        </template>
      </avue-crud>
    </basic-container>
...
import { getSafetyStockPageApi, exportSafetyStockApi } from "@/api/warehouse/safetyStock"
...
export default {
  ...
  data() {
    return {
      ...
      brandOption: [],
      categoryId: [],
      newDownloadUrl: "/warehouse/wmssafestock/downloadTemplate",
      newPreviewUrl: "/warehouse/wmssafestock/batchImportPreview",
      newUploadUrl: "/warehouse/wmssafestock/batchImportData",
      newDownloadTitle: "海外仓安全库存配置"
    }
  },
  created() {},
  mounted: function () {
  ...
  methods: {
    ...
    // 导出
    async handleExport() {
      const { data } = await exportSafetyStockApi({ ...this.searchForm, ...this.page, type: 0 })
