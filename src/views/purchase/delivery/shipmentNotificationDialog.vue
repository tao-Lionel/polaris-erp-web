        ...
        <span v-for="(item, index) in purchaseForm.shippingNoticeNoList" :key="index">
          ...
          >
            <el-input
              ...
              clearable
            ></el-input>
            <i style="margin-left: 10px" class="el-icon-plus" v-if="index == 0" @click="handleAdd"></i>
            <i style="margin-left: 10px" class="el-icon-minus" v-if="index > 0" @click="handleMinus(index)"></i>
          </el-form-item>
        </span>
      </el-form>
    </div>
...
export default {
  ...
  methods: {
    openDialog(row) {
      this.visible = true
      this.purchaseForm.deliveryNo = row.deliveryNo
    ...
    handleMinus(index) {
      if (
        this.purchaseForm.shippingNoticeNoList.length > 1 &&
        index >= 0 &&
        index < this.purchaseForm.shippingNoticeNoList.length
      ) {
        this.purchaseForm.shippingNoticeNoList.splice(index, 1)
      }
    },
    handleConfirm() {
      this.$refs.form.validate((vaild) => {
        if (vaild) {
          const postData = {
            deliveryNo: this.purchaseForm.deliveryNo,
