...
const common = {
  state: {
    ...
    theme: getStore({ name: "theme" }) || "#46B1B0",
    themeName: getStore({ name: "themeName" }) || "",
    lockPasswd: getStore({ name: "lockPasswd" }) || "",
    website: website,
    downloadCount: getStore({ name: "downloadCount" }) || "",
    warehouseStockChange: getStore({ name: "warehouseStockChange" }) || [],
    syncCalcFiltering: getStore({ name: "syncCalcFiltering" }) || ""
  },
  actions: {},
  mutations: {
    ...
    SET_DOWNLOAD_COUNT: (state, downloadCount) => {
      state.downloadCount = downloadCount
      setStore({
        name: "downloadCount",
        content: state.downloadCount
      })
    },
    //
    WAREHOUSE_STOCK_CHANGE: (state, warehouseStockChange) => {
      state.warehouseStockChange = warehouseStockChange
      setStore({
        name: "warehouseStockChange",
        content: state.warehouseStockChange
      })
    },
    SYNC_CALC_FILTERING: (state, syncCalcFiltering) => {
      state.syncCalcFiltering = syncCalcFiltering
      setStore({
        name: "syncCalcFiltering",
        content: state.syncCalcFiltering
      })
    }
  }
}
