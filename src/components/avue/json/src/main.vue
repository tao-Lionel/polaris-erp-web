<template>
  <span class="json-tree" :class="{ 'json-tree-root': parsed.depth === 0 }">
    <span class="json-tree-row" v-if="parsed.primitive" @mouseover="hovered = true" @mouseout="hovered = false">
    ...
    <span class="json-tree-deep" v-if="!parsed.primitive">
      <span class="json-tree-row json-tree-expando" @mouseover="hovered = true" @mouseout="hovered = false">
        <span class="json-tree-indent">&nbsp;</span>
        <span class="json-tree-sign" @click="expanded = !expanded">{{ expanded ? "-" : "+" }}</span>
