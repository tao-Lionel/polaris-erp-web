 ...
 * 对象深拷贝，但忽略掉null值
 */
export const deepCloneIgnoreNull = (data) => {
  var type = getObjType(data)
  var obj
  if (type === "array") {
    obj = []
  ...
  if (!validatenull(data)) {
    if (type === "array") {
      for (var i = 0, len = data.length; i < len; i++) {
        if (!validatenull(data[i])) {
          obj.push(deepCloneIgnoreNull(data[i]))
        }
      }
    } else if (type === "object") {
      for (var key in data) {
        if (!validatenull(data[key])) {
          obj[key] = deepCloneIgnoreNull(data[key])
        }
      }
    }
  }
...
export const initVal = ({ type, multiple, value, listType }) => {
  if (
    ...
    if (!Array.isArray(value) && value != "") return (value || "").split(",")
    else return value
  } else {
    return value
  }
