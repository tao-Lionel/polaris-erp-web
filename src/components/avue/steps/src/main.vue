  ...
  <div :class="b()">
    ...
    >
      <el-step
        ...
        :key="index"
        @click.native="option.switchBtn ? switchs(index) : ''"
      ></el-step>
    </el-steps>
    <div :class="b('contail')">
      ...
      <div v-if="crudFlag">
        ...
        <div :class="b('menu')">
          <el-button @click="submit" icon="el-icon-back" type="primary">{{ lastFlag ? "提交" : "下一步" }}</el-button>
          <el-button @click="breaks" icon="el-icon-back" v-if="formIndex != 1 && vaildData(option.breakBtn, true)"
            >返 回</el-button
          >
        </div>
      </div>

      <avue-form v-else :option="formOption" :class="b('form')" @submit="submit" v-model="text">
        <template slot-scope="scope" v-for="item in formColumnOption" :slot="item.prop">
          <slot
            :value="scope.value"
            :column="scope.column"
